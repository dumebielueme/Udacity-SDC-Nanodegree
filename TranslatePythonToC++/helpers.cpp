...
/*
-------------------------NORMALIZE FUNCTION--------------------------------
This Normalize function is a conversion of the Normalize function in attached helpers.py
Variables are kept about the same for convenience
*/
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// todo - your code here
  	float total = 0; // total variable to hold the sum of values
  	//int row = grid.size();
  	//int col = grid[0].size();

	for (int row = 0; row < grid.size(); row++){ //python: for row in grid
		vector<float> new_row = grid[row];

	for (int col = 0; col < grid[0].size(); col++){ //python: for cell in row
		total += new_row[col];
		}
	}

	if (total == 0) //checking that the total value is not zero because a number diveded by zero is undefined
	{
		throw std::logic_error("Normalization, not possible. Cannot divide by zero"); //can't divide by zero
	}
	//iterating through again
	for (int row = 0; row < grid.size(); row++){// python: enumerate(grid)
		vector<float> new_row = grid[row];

	for (int col = 0; col < grid[0].size(); col++){
		newRow.push_back(new_row[col] / total);
		}

	newGrid.push_back(newRow); // pushback the row generated
	newRow.clear();
	}
...
/*
-------------------------BLUR FUNCTION--------------------------------
This Blur function is a conversion of the Blur function in attached helpers.py
*/
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	float center_prob = 1.0 - blurring; //formulas provided in Histogram Project
	...
	for (int row = 0; row < height; row++) { //iterating through the matrix rows and columns
		for (int col = 0; col < width; col++) {
          float grid_val = grid[row][col];
			for (int dx = -1; dx < 2; dx++) {
				for (int dy = -1; dy < 2; dy++) { 
                  	float mult = window[dx + 1][dy + 1];
					int new_row = (height + row + dy) % height;
					int new_col = (width + col + dx) % width;
