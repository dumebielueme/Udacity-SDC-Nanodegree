...
*/
/*
-------------------------INITIALIZE BELIEFS FUNCTION--------------------------------
This Initialize Beliefs function is a conversion of the Initialize Beliefs function in attached localizer.py
*/
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	vector< vector <float> > newGrid; //newGrid is beliefs in localizer.py
...
/*
-------------------------MOVE FUNCTION--------------------------------
This Move function is a conversion of the Move function in attached localizer.py
*/
vector< vector <float> > move(int dy, int dx, vector < vector <float> > beliefs, float blurring) {
  ...
  // your code here
	...
	for (int row = 0; row < height; row++) {
    for (int col = 0; col < width; col++) {
		int new_row = (row + dy) % width;
		int new_col = (col + dx) % height;
		float cell = beliefs[row][col];
		newGrid[new_row][new_col] += cell;
		}
    }
...
/*
-------------------------SENSE FUNCTION--------------------------------
This Sense function is a conversion of the Sense function in attached localizer.py
*/
	...
	vector< vector <float> > beliefs, float p_hit, float p_miss) 
{
	...
	for (int row = 0; row < height; row++) {
		vector<float> new_row;
    for (int col = 0; col < width; col++) {
		if(grid[row][col] == color){ //if grid color = provided color, pHit
			new_row.push_back((beliefs[row][col]) * p_hit);
								}
		else {		//if grid color =/= provided color, pMiss
